...
#for(i in 2:26) lines(density(na.omit(expr1[,i])), col=col_vec[i])
...
#after quantile transformation
plot(density(na.omit(expr.qt[ ,1])), col = col_vec[1])
for(i in 2:40) lines(density(na.omit(unlist(expr.qt[ ,i]))), col=col_vec[i])
...
### PCA for plink genotype
# read in data
pca <- read_table2("genotype/input/directory/pangenie_svs_sample_all.eigenvec", col_names = FALSE)
eigenval <- scan("genotype/input/directory/pangenie_svs_sample_all.eigenval")
...
Superpopulation = c('EAS', 'EAS', 'AFR', 'AFR', 'AMR', 'AFR', 'EUR', 'SAS', 'EUR', 'EUR', 'EAS', 'AMR', 'EAS', 'AMR', 'EUR', 
                    'AMR', 'EAS', 'AFR', 'EAS', 'SAS', 'AFR', 'SAS', 'AFR', 'AFR', 'SAS', 'SAS', 'EAS', 'EAS', 'AMR', 'AMR', 
                    'AFR', 'AFR', 'AFR', 'AFR', 'AFR', 'AFR', 'AFR', 'AFR', 'AFR', 'AFR')
Phase = c('HGSVC2', 'HGSVC2','HGSVC2','HGSVC2','HGSVC2','HGSVC2', 'HGSVC2', 'HGSVC2', 'HGSVC2', 'HGSVC2',
          'HGSVC2','HGSVC2','HGSVC2','HGSVC2', 'HGSVC2','HGSVC2','HGSVC2', 'HGSVC2', 'HGSVC2', 'HGSVC2', 
          'HGSVC2','HGSVC2', 'HGSVC2','HGSVC2', 'HGSVC2', 'HGSVC2', 'hgsvc1', 'hgsvc1', 'hgsvc1', 'hgsvc1', 
          'hgsvc1', 'hgsvc1', 'hgsvc1', 'hgsvc1', 'hgsvc1', 'hgsvc1', 'hgsvc1', 'hgsvc1', 'hgsvc1', 'hgsvc1')
Superpopulation_Phase <- paste0(Superpopulation, "_", Phase)

# remake data.frame
pca <- as.tibble(data.frame(pca, Superpopulation, Phase, Superpopulation_Phase))
# first convert to percentage variance explained
pve <- data.frame(PC = 1:20, pve = eigenval/sum(eigenval)*100)
pve <- data.frame(PC = 1:20, pve = eigenval/sum(eigenval))
# make plot
a <- ggplot(pve, aes(PC, pve)) + geom_bar(stat = "identity")
a + ylab("Percentage variance explained") + theme_light()
a + ylab("PVE") + theme_light()

cumsum(pve$pve)
# plot pca
b <- ggplot(pca, aes(PC1, PC2, col = Superpopulation, shape = Phase)) + geom_point(size = 3)
b <- b + scale_colour_manual(values = c('gold', 'red', "green", "blue", 'purple'))
